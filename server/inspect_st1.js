...
async function inspectStudent1() {
    ...
    const grades = await callOdoo('object', 'execute_kw', [
        ...
        [[['student_id', '=', 1]]],
        { fields: ['id', 'subject_id', 'sub_subject_id', 'semester', 'semester_id', 'year_id'] }
    ]);
    console.log(`Total grades: ${grades.length}`);
    grades.forEach(g => {
        console.log(`ID ${g.id}: Subj=${g.subject_id[1]} | SubSubj=${g.sub_subject_id ? g.sub_subject_id[1] : 'NONE'} | Sem=${g.semester} | SemID=${g.semester_id} | YearID=${g.year_id}`);
    });
}
